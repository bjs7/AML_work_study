...
def gnn_trainer(args, train_data, vali_data, model_configs, seed = None):
    ...
    m_param['num_neighbors'] = [100, 100, 100]
    m_param['num_neighbors'] = [10, 10, 10]
    m_param['dropout'] = 0
    ...
    for epoch in range(epochs):
        ...
        for batch in tqdm.tqdm(train_loader):
            ...
            if m_settings['index_masking']:
                ...
                mask1 = torch.isin(batch.input_id, batch.edge_attr[:, 0].detach().cpu().to(torch.int))
                sum(mask)

                np.where(batch.edge_label == 1)
                batch.input_id[337]
                train_data.y[1188]

                train_data.edge_attr[1188,0:2]
                train_data.edge_attr[1188,19]

                train_data.edge_attr[batch.input_id, :][337,0:2]
                train_data.edge_attr[batch.input_id, :][337,19]
                ...
                sorted(out[mask][:,0])[0:10]
                pred = out[mask]
                test123 = pred
                ground_truth = batch.y[mask]
                np.where(batch.y[mask] == 1)
            else:
                batch.edge_attr = batch.edge_attr[:, 1:] if m_settings['include_time'] else batch.edge_attr[:, 2:]
                target_edge_attr = target_edge_attr[:, 1:] if m_settings['include_time'] else target_edge_attr[:, 2:]
                import utils
