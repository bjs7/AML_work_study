...
def gnn_trainer(args, train_data, vali_data, model_configs, seed = None):
    ...
    m_param['num_neighbors'] = [10000]
    train_loader = LinkNeighborLoader(train_data, num_neighbors=m_param.get('num_neighbors'), edge_label = train_data.y, batch_size=batch_size, shuffle=True, transform=None)
    train_loader = LinkNeighborLoader(train_data, num_neighbors=[-1], edge_label = train_data.y, batch_size=10007, shuffle=False, transform=None)
    sample_batch = next(iter(train_loader))
    train_data.edge_index
    sample_batch.edge_label_index
    sample_batch.edge_attr[0:10,0].to(torch.int)
    train_data.edge_attr[0:10,0]
...
def train_homo(train_loader, train_data, train_indices, vali_loader, vali_data, pred_indices, model, optimizer, loss_fn, device, m_settings, args):
    ...
    target_edge_attr = train_data.edge_attr.to(device)
    ...
    for epoch in range(epochs):
        ...
        pred = model(train_data.x, train_data.edge_index, train_data.edge_attr, train_data.edge_index, target_edge_attr, index_mask = True)
