...
def save_results(results, hyperparams, manager):
    """Save experiment results with comprehensive config tracking."""
    
    # ... (same base_dir creation as before) ...
    
    # Create unique run directory
    run_id = _get_next_run_id(base_dir)
    save_dir = os.path.join(base_dir, f"run_{run_id:03d}")
    Path(save_dir).mkdir(parents=True, exist_ok=True)
    
    # Save all experiment files
    _save_config(save_dir, manager, hyperparams)
    _save_metrics(save_dir, results)
    _save_predictions(save_dir, results)
    # ... save models ...
    
    # Update master index
    _update_experiment_index(save_dir, manager, hyperparams, results)
    
    print(f"✅ Results saved to: {save_dir}")
    return save_dir


def _update_experiment_index(save_dir, manager, hyperparams, results):
    """Update master experiment index for easy querying."""
    index_file = os.path.join(config.save_direc_training, 'experiment_index.json')
...
def find_experiments(filter_dict=None, sort_by='f1', top_n=None):
    """Find experiments matching criteria.
    
    Usage:
        # Find all experiments with ibm_fe=True
        exps = find_experiments({'ibm_fe': True})
        
        # Find best full_info experiment
        best = find_experiments({'fl_algo': 'full_info'}, sort_by='f1', top_n=1)
        
        # Find all individual experiments with specific settings
        exps = find_experiments({
            'fl_algo': 'individual',
            'model': 'GINe',
            'train_for_final': False
        })
    """
    index_file = os.path.join(config.save_direc_training, 'experiment_index.json')
...
def load_experiment(filter_dict, sort_by='f1'):
    """Load best experiment matching criteria.
    
    Usage:
        # Load best individual GINe experiment
    ...
    """
    exps = find_experiments(filter_dict, sort_by=sort_by, top_n=1)
    
    if not exps:
        print(f"No experiments found matching: {filter_dict}")
        return None
    
    exp = exps[0]
    print(f"Loading: {exp['run_path']}")
    print(f"F1: {exp['metrics']['f1']:.4f}, Timestamp: {exp['timestamp']}")
    
    return load_results(exp['run_path'])


def compare_experiments(filter_dict=None):
    """Compare all experiments matching criteria as DataFrame.
    
    Usage:
        # Compare all individual experiments
        df = compare_experiments({'fl_algo': 'individual'})
    ...
    exps = find_experiments(filter_dict)
...
# Find best individual GINe experiment
results = load_experiment({
    'fl_algo': 'individual',
    'model': 'GINe',
    'train_for_final': False
...
# Compare all full_info experiments
df = compare_experiments({'fl_algo': 'full_info'})
print(df[['run_id', 'ibm_fe', 'train_for_final', 'f1', 'accuracy']])

# Find all experiments with F1 > 0.8
exps = find_experiments()
high_f1 = [e for e in exps if e['metrics']['f1'] > 0.8]
